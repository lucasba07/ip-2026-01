    ...
    <style>
        ...
        * {
        ...
        .card {
            ...
            border-top: 4px solid var(--action-color, #22A06B);
            animation: slideUp 0.38s ease both;
            position: relative;
        }
        .card[data-color="#10b981"] { --action-color: #22A06B; }
...
func CreatePatientHandler(w http.ResponseWriter, r *http.Request) {
	...
	err := r.ParseForm()
	if err != nil {
		fmt.Fprintf(w, responseTemplate, "#ef4444", "Erro", "Erro ao processar o formulário.")
		return
	}
	...
	err = utils.CreatePatient(name, cpf, birthDate, bloodType, phone, email, medicalNotes)
	if err != nil {
		fmt.Fprintf(w, responseTemplate, "#ef4444", "Erro ao cadastrar", err.Error())
		return
	}

	fmt.Fprintf(w, responseTemplate, "#10b981", "Sucesso!", "Paciente cadastrado com sucesso no sistema.")
}
